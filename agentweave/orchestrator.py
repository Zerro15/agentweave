...
from .collaboration import CollaborationEngine, ConflictResolver, ConsensusEngine
from .engine import AgentMatcher, AgentRegistry, PlacementEngine, TrustEngine
...
class AgentWeave:
    def __init__(
        ...
        *,
        ...
        self.registry = AgentRegistry(self.store, self.graph)
        self.analyzer = RequirementAnalyzer()
        self.trust = TrustEngine(self.store)
        self.placement = PlacementEngine()
        self.matcher = AgentMatcher(self.trust, self.placement, use_native=use_native)
        self.selector = GlobalTeamOptimizer()
        ...
        self.semantic_verifier = semantic_verifier or SemanticResultVerifier()
        self.retest_policy = RetestPolicy()
        self.a2a = a2a or InMemoryA2AAdapter()
        self.collaboration = CollaborationEngine(self.a2a)
        self.consensus = ConsensusEngine()
        self.conflicts = ConflictResolver(self.a2a)
        self.policy = policy_engine or GovernancePolicyEngine()
        self.observability = Observability(self.store)
        self.interop = A2AInteropSuite()
        self.lifecycle = LongRunningA2AClient()
        self.push_notifications = PushNotificationConfigClient()
        ...
        self.did = DIDResolver()
        self.vc = VerifiableCredentialVerifier()
        self.revocations = RevocationRegistry()
        self.cert_rotation = CertificateRotationManager()
        self.keys = KeyManager()
        self.attestation = WorkloadAttestationVerifier()
        self.sandbox = DockerSandbox()
        self.sandbox_policy = SandboxPolicy()

    def _register_internal(self, agent, *, trust_path: str, reason: str | None = None):
        with self.observability.tracer.span(
            "agentweave.registry.register",
    ...
    def register_trusted(self, agent, *, reason: str = "caller-asserted-trust"):
        """Register an agent that the caller already trusts.

        This path deliberately skips untrusted onboarding checks. The audit record makes
        that trust assertion explicit so it cannot be confused with validated discovery.
        """

        return self._register_internal(agent, trust_path="trusted", reason=reason)

    def register(self, agent):
        """Backward-compatible alias for explicit trusted registration.

        New applications should call ``register_trusted`` or ``onboard_untrusted`` so
        the trust boundary is visible in code review and audit records.
        """

        return self.register_trusted(agent, reason="legacy-register-api")

    async def onboard_untrusted(
        self,
        agent,
        *,
        ...
        """Validate an untrusted discovered agent before registry admission."""

        with self.observability.tracer.span(
            "agentweave.onboarding.validate",
            agent_id=agent.agent_id,
        ):
            sec = (
                self.security.validate(agent)
                if security_check
                else {"passed": True, "score": 1.0, "problems": [], "warnings": []}
            )
            identity = {"passed": True, "verified": False}
            card = agent.metadata.get("agent_card")
            if self.revocations.is_revoked(agent.agent_id):
                identity = {"passed": False, "verified": False, "error": "agent-revoked"}
            elif require_signed_card:
                identity = {
                    "passed": False,
                    "verified": False,
                    "error": "signed-agent-card-required",
                }
                if card and key_resolver:
                    try:
                        key = key_resolver(agent, card)
                        claims = self.identity.verify_agent_card_jws(card, key) if key else None
                        if claims:
                            identity = {"passed": True, "verified": True, "claims": claims}
                            agent.trust.identity = 1.0
                            agent.signature_verified = True
                    except Exception as exc:
                        identity = {"passed": False, "verified": False, "error": str(exc)}
                        agent.trust.identity = 0.0
            ...
            if registered:
                self._register_internal(
                    agent,
                    trust_path="validated-untrusted",
                    reason="security+identity+policy-passed",
                )
            verdict = {
                "agent_id": agent.agent_id,
                "security": sec,
    ...
    async def ingest_marketplace(
        ...
        *,
        ...
        for agent in agents:
            verdicts.append(
                await self.onboard_untrusted(
                    agent,
                    security_check=security_check,
                    require_signed_card=require_signed_cards,
    ...
    async def benchmark_agent(self, agent_id, cases, threshold=.7):
        ...
        validator = BenchmarkValidator(
            lambda a, p: self.a2a.invoke(a, p, context={"mode": "benchmark"})
        )
        ...
        self._register_internal(
            agent,
            trust_path="existing-agent-update",
    ...
    async def retest_due_agents(self, benchmark_factory, threshold=.7):
        validator = BenchmarkValidator(
            lambda a, p: self.a2a.invoke(a, p, context={"mode": "retest"})
        )
    ...
    async def solve(
        ...
        *,
        ...
        with self.observability.tracer.span(
            "agentweave.requirement.analyze",
            requirement=text,
        ):
        ...
        policy_decisions = {}
        with self.observability.tracer.span(
            "agentweave.policy.filter",
            candidate_count=len(self.registry.all()),
        ):
            for agent in self.registry.all():
                decision = self.policy.evaluate(
                    agent,
                    req,
                    policy_context or PolicyContext(),
                )
                policy_decisions[agent.agent_id] = decision.__dict__
                if decision.allowed and not self.revocations.is_revoked(agent.agent_id):
                    candidates.append(agent)
        with self.observability.tracer.span(
            "agentweave.match.rank",
            requirement=text,
            candidates=len(candidates),
        ):
            ranked = self.matcher.rank(req, candidates)
        with self.observability.tracer.span(
            "agentweave.team.select",
            max_agents=max_agents,
        ):
            team = self.selector.select(req, ranked, max_agents=max_agents)
        explanation = self.observability.explainer.explain(
            req,
            ranked,
            team,
            policy_decisions,
        ...
        self.observability.audit.record(
            "team.selected",
            payload={"agents": [m.agent.agent_id for m in team]},
        )
        with self.observability.tracer.span(
            "agentweave.a2a.collaboration",
            rounds=rounds,
            team_size=len(team),
        ):
            collaboration_started = time.perf_counter()
            ...
            self.observability.metrics.observe(
                ...
                (time.perf_counter() - collaboration_started) * 1000,
            )
        final_round = max((r["round"] for r in transcript), default=0)
        final_results = [r for r in transcript if r["round"] == final_round]
        ...
        with self.observability.tracer.span("agentweave.conflict_resolution"):
            resolution = await self.conflicts.resolve(
                effective_team,
                text,
                final_results,
                consensus,
        ...
        with self.observability.tracer.span("agentweave.result_validation"):
            ...
            semantic = None
            if semantic_verify:
                semantic = await self.semantic_verifier.verify(final_results, text)
                validation["semantic"] = semantic
                validation["score"] = 0.65 * validation["score"] + 0.35 * semantic["score"]
                validation["passed"] = validation["passed"] and semantic["score"] >= 0.45
        per_agent = {r["agent_id"]: r for r in final_results}
        with self.observability.tracer.span(
            "agentweave.reputation.update",
            agents=len(effective_team),
        ):
            for member in effective_team:
                result = per_agent.get(member.agent.agent_id, {})
                success = bool(result.get("success")) and validation["passed"]
                self.trust.update(
                    member.agent,
                    success,
                    validation["score"],
        ...
        return {
            ...
            "capability_coverage": self.graph.coverage(
                req,
                [m.agent for m in effective_team],
            ),
            "native_acceleration": self.matcher.native_available,
            "policy": policy_decisions,
            "selection_explanation": explanation,
