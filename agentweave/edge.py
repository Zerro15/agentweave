...
class EdgeRuntime:
    async def invoke(self, agent: AgentProfile, prompt: str) -> dict:
        raise NotImplementedError


class LlamaCppRuntime(EdgeRuntime):
    def __init__(self, binary="llama-cli"):
        self.binary = binary

    async def invoke(self, agent, prompt):
        model = agent.metadata.get("model_path")
        if not model:
            raise ValueError("model_path missing")
        if not shutil.which(self.binary):
            raise RuntimeError(f"{self.binary} not installed")
        proc = await asyncio.create_subprocess_exec(
            ...
            prompt,
            "-n",
            str(agent.metadata.get("max_tokens", 256)),
...
class OllamaRuntime(EdgeRuntime):
    def __init__(
        ...
        *,
        ...
        self.transport = transport or SafeHttpTransport(timeout=120)
...
class EdgeA2AAdapter:
    def __init__(self, default_runtime=None):
        self.default_runtime = default_runtime or OllamaRuntime()
        self.runtimes = {}
    ...
    async def invoke(self, agent, task, context=None):
        return await self.runtimes.get(
            agent.agent_id,
