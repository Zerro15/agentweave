...
class ProtobufCodec:
    ...
    def __init__(self, pb2):
        ...
        self._parse = ParseDict

    def build(self, class_name, payload):
        cls = getattr(self.pb2, class_name)
        return self._parse(payload, cls(), ignore_unknown_fields=False)

    def to_dict(self, message):
        return self._to_dict(message, preserving_proto_field_name=False)
...
class GrpcA2ALifecycleClient:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError("Install agentweave-router[grpc]") from exc
        self.grpc = grpc
        self.target = target
        self.timeout = timeout
        self.codec = ProtobufCodec(pb2)
        self.channel = (
    ...
    async def _unary(self, method, request_name, payload):
        response = await getattr(self.stub, method)(
            self.codec.build(request_name, payload),
            timeout=self.timeout,
        )
        return self.codec.to_dict(response)
    ...
    async def stream(
        ...
        if tenant:
            payload["tenant"] = tenant
        call = getattr(self.stub, "SendStreamingMessage")(
            self.codec.build("SendMessageRequest", payload),
            timeout=self.timeout,
        )
        async for event in call:
            yield self.codec.to_dict(event)

    async def subscribe(self, task_id, tenant=None) -> AsyncIterator[dict]:
        payload = {"id": task_id}
        if tenant:
            payload["tenant"] = tenant
        call = getattr(self.stub, "SubscribeToTask")(
            self.codec.build("SubscribeToTaskRequest", payload),
            timeout=self.timeout,
        )
        async for event in call:
            yield self.codec.to_dict(event)
    ...
    async def get_task(self, task_id, history_length=None, tenant=None):
        ...
        if history_length is not None:
            payload["historyLength"] = history_length
        if tenant:
            payload["tenant"] = tenant
        return await self._unary("GetTask", "GetTaskRequest", payload)

    async def list_tasks(self, **filters):
        return await self._unary("ListTasks", "ListTasksRequest", filters)
...
class PushNotificationConfigClient:
    ...
    def _interface(self, agent):
        ...
        binding = str(
            ...
            or iface.get("protocolBinding")
            or "JSONRPC"
        ).upper()
        if not endpoint:
            raise ValueError("agent endpoint missing")
    ...
    async def set(self, agent, task_id, config):
        ...
        if binding in {"HTTP+JSON", "HTTP_JSON", "REST"}:
            response = await self.transport.post(
                f"{endpoint}/tasks/{task_id}/pushNotificationConfigs",
                json=payload,
                headers={
    ...
    async def get(self, agent, task_id, config_id):
        ...
        if binding in {"HTTP+JSON", "HTTP_JSON", "REST"}:
            response = await self.transport.get(
                f"{endpoint}/tasks/{task_id}/pushNotificationConfigs/{config_id}",
    ...
    async def delete(self, agent, task_id, config_id):
        ...
        if tenant:
            payload["tenant"] = tenant
        if binding in {"HTTP+JSON", "HTTP_JSON", "REST"}:
            response = await self.transport.delete(
                f"{endpoint}/tasks/{task_id}/pushNotificationConfigs/{config_id}",
                headers={"A2A-Version": self.protocol_version},
            )
            response.raise_for_status()
...
class PushNotificationReceiver:
    ...
    def __init__(
        self,
        secret=None,
        signature_header="x-agentweave-signature",
    ...
    def app(self):
        try:
            ...
            raise RuntimeError("Install agentweave-router[api]") from exc
        ...
        async def receive(request: Request):
            ...
            await self.queue.put(
                PushEvent(payload=payload, signature_verified=verified)
            )
            return JSONResponse({"accepted": True}, status_code=202)

        return Starlette(routes=[Route("/a2a/push", receive, methods=["POST"])])

    async def next_event(self, timeout=30):
