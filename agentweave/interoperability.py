...
def _capture_value(obj, spec):
    ...
    for path in candidates:
        ...
        if value is not None:
            return value
    leaf_names = [str(item).split(".")[-1] for item in candidates]
...
class A2AInteropSuite:
    def __init__(
        ...
        *,
        ...
        self.adapter = adapter or HttpA2AAdapter(transport=self.transport)

    async def _http_bootstrap(self, target, runtime_headers):
        boot = target.http_bootstrap or {}
        method = str(boot.get("method", "POST")).upper()
        url = boot["url"]
        body = boot.get("json")
        ...
        for header, spec in (boot.get("capture_headers") or {}).items():
            ...
            if value is None:
                raise RuntimeError(
                    f"HTTP bootstrap response missing required credential for {header}"
                )
    ...
    async def run_target(
        ...
        prompt="A2A interoperability test",
    ):
        ...
        try:
            ...
            if target.bootstrap:
                ...
                result = await self.adapter.rpc_call(
                    agent,
                    boot["method"],
                    boot.get("params") or {},
                ...
                for header, spec in (boot.get("capture_headers") or {}).items():
                    ...
                    if value is None:
                        raise RuntimeError(
                            f"Bootstrap response missing required credential for {header}"
                        )
                    runtime_headers[header] = str(value)
            if target.message is not None:
                await self.adapter.invoke_message(
                    agent,
                    target.message,
                    rpc_method=target.rpc_method,
                    context={
                        "test": "agentweave-interop",
            ...
            return InteropResult(
                ...
                agent.name,
                transport,
                bool(agent.metadata.get("streaming")),
            )
        except Exception as exc:
            return InteropResult(
                target.name,
    ...
    async def run(self, targets: list[InteropTarget], prompt=None):
        return await asyncio.gather(
            *(
                self.run_target(target, prompt or "A2A interoperability test")
    ...
    def from_env(name="AGENTWEAVE_A2A_TARGETS"):
        ...
        if not raw:
            return []
        return [InteropTarget(**item) for item in json.loads(raw)]


class A2ATCKRunner:
    def __init__(self, tck_dir: str | None = None):
        self.tck_dir = tck_dir
    ...
    def run(
        ...
        if not self.tck_dir:
            raise ValueError("tck_dir is required")
...
async def probe_well_known(
    ...
    *,
    ...
    safe = transport or SafeHttpTransport(timeout=15)
