...
class LongRunningA2AClient:
    ...
    All AgentWeave-owned HTTP traffic uses ``SafeHttpTransport`` including SSE
    streams, so lifecycle operations share the same SSRF/redirect/DNS policy as the
    normal A2A transport.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.store = state_store or TaskStateStore()
    ...
    async def send(self, agent, message, context=None, task_id=None):
        ...
        local_id = task_id or str(uuid.uuid4())
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if remote_id != state.task_id:
                    self.store.tasks.pop(state.task_id, None)
                    state.task_id = str(remote_id)
                state.status = self._status(data) or "completed"
                state.result = data
                state.error = None
                self.store.save(state)
                return state
    ...
    async def stream(
        ...
        async for data in self._sse(url, payload, headers, method="POST"):
            yield data

    async def subscribe(self, agent, task_id) -> AsyncIterator[dict]:
        endpoint, binding, tenant = self._interface(agent)
        if binding in {"HTTP+JSON", "HTTP_JSON", "REST"}:
            url = endpoint + "/tasks/" + task_id + ":subscribe"
            payload = None
            headers = {
                "Accept": "text/event-stream",
                "A2A-Version": self.protocol_version,
            }
            async for data in self._sse(url, payload, headers, method="GET"):
                yield data
        else:
            params = {"id": task_id}
            if tenant:
            ...
            async for data in self._sse(endpoint, payload, headers, method="POST"):
                yield data
    ...
    async def list_tasks(self, agent, **filters):
        endpoint, binding, tenant = self._interface(agent)
        if tenant:
            filters.setdefault("tenant", tenant)
        if binding in {"HTTP+JSON", "HTTP_JSON", "REST"}:
            response = await self.transport.get(
                endpoint + "/tasks",
                params=filters,
    ...
    async def resume(self, agent, task_id, poll_interval=0.5, timeout=None):
        deadline = time.monotonic() + (timeout or self.timeout)
        while True:
            data = await self.get_task(agent, task_id)
    ...
    def _status(data: Any):
        ...
        if status is None and isinstance(data.get("task"), dict):
            return LongRunningA2AClient._status(data["task"])
        if status is None:
            return None
